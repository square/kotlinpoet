...
private constructor(
  ...
  public companion object {
    ...
    public operator fun invoke(
      ...
      of(name, bounds.map { it.asTypeName() }.ifEmpty(::NULLABLE_ANY_LIST), variance)
     ...
     * infinite recursion in cases like `Enum<E extends Enum<E>>`. When we encounter such a thing,
     * we will make a TypeVariableName without bounds and add that to the `typeVariables` map before
     * looking up the bounds. Then if we encounter this TypeVariable again while constructing the
     * bounds, we can just return it from the map. And, the code that put the entry in `variables`
