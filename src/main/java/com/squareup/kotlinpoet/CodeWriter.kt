...
internal class CodeWriter constructor(
  ...
  private fun emitValue(o: Any?) {
    ...
    when (o) {
      ...
      is Array<*> -> emitIterable(o.asIterable(), "arrayOf")
      is List<*> -> emitIterable(o, "listOf")
      is Set<*> -> emitIterable(o, "setOf")
      is Map.Entry<*, *> -> emitCode("%V to %V", o.key, o.value)
      is Map<*, *> -> emitIterable(o.entries, "mapOf")
      is Pair<*, *> -> emitCode("%V to %V", o.first, o.second)
      else -> throw UnsupportedOperationException("Value type ${o::class} not supported.")
    }
  }

  private fun emitIterable(iterable: Iterable<*>, prefix: String) {
    emit("$prefix(")
    iterable.forEachIndexed { i, any ->
      if (i > 0) emit(", ")
      emitValue(any)
    }
    emit(")")
